...
for idx, filename in enumerate(os.listdir(folder_path)):
    if filename.endswith('.json'):  
        ...
        try:
            ...
            iata_code = df['departure.iataCode'].iloc[0].lower()  
            numeric_iata = iata_mapping.get(iata_code, -1)  
            
            # Create a summary for this file
            summary = {
                'departure.iataCode': iata_code.upper(),       # Store the original IATA code
                'departure.iataCode_numeric': numeric_iata,   
                'date': df['date'].iloc[0],                  
...
save_path = r"C:\...\summary_airport.csv"
